...
空格键的默认动作是，当鼠标选中control bar的播放键时，空格键可以切换播放/暂停；当鼠标选中全屏键时，空格键可以切换全屏；当鼠标选中静音键时，空格键可以切换静音；当鼠标什么也没选中时，空格键无法控制播放器的任何功能。
...
对于后者，涉及到[keyCode/键码]()的知识。具体到这个案例，空格键的键码是32。
...
由于JS的[事件冒泡]()机制，onKeyDown事件是可以成功捕获的。
...
有没有更好的方法？仔细查阅文档，发现[events文档](http://docs.videojs.com/docs/api/player.html#events)中的这一条：
...
根据该插件的功能，将其命名为setStateandFocusPlugin，完整代码如下
